# 🌍 Excel Data Analysis Project by Akanksha Kamble 

## 📊 Europe Sales Dashboard  
  

## 📌 Overview  
This project presents a detailed **Europe Sales Analysis Dashboard** designed to deliver insights into sales performance across multiple European countries. The dashboard provides a comprehensive view of **revenue, sales channels, product categories, and order priorities**, empowering decision-makers to identify trends, optimize operations, and maximize profitability.  

---

## 🛠️ Tool Used  
![Excel_Image](https://github.com/user-attachments/assets/4602f4ce-bebf-4b75-a424-8225f4d2883d)

---

## 🎯 Key Performance Indicators (KPIs)  
The dashboard tracks the following business-critical KPIs:  

- 💰 **Total Revenue** – Profitability across European countries  
- 📦 **Units Sold** – Quantity of products sold by item type  
- 🌍 **Top Profitable Countries** – Highest revenue-generating regions  
- 🛒 **Sales Channels** – Online vs. Offline contribution  
- 🏷️ **Item Type Analysis** – Category-wise sales distribution (Office Supplies, Beverages, Fruits, Clothes, Vegetables, Baby Food)  
- 📑 **Order Priority Performance** – Contribution of High, Medium, Low, and Critical orders  
- 📈 **Country-Wise Sales Trends** – Comparison across European regions

 ---

 ## 📈 Key Insights  
- 🥇 **Ukraine** emerged as one of the most profitable countries.  
- 💰 **Top 10 countries** generated revenue exceeding **5M+**.  
- 📦 Highest number of **units sold** comes from **Office Supplies and Beverages**.  
- 🏷️ **Order Priority "M" (Medium)** contributed the **largest share of sales**.  
- 🔄 Both **Online and Offline** channels significantly contribute to overall sales distribution.  

---

## 🔍 Additional Insights  
- 📊 **Revenue concentration** is high in a few countries – expansion opportunities exist in lower-performing regions.  
- 📦 **Critical priority orders (C)**, while fewer, still form a substantial part of the sales volume.  
- 🛍️ **Balanced demand** across multiple item categories shows a diversified product portfolio.  
- 📉 Some countries show **low revenue despite high sales volume**, suggesting possible pricing or cost challenges.  

---

## 📚 Data Story  
- 🌍 **Ukraine, Kosovo, and San Marino** lead as **top-performing countries**.  
- 📦 **Office Supplies and Beverages** dominate sales volume, indicating steady demand.  
- 📉 Countries like **Andorra** and **Bosnia-Herzegovina** underperform compared to peers.  
- 💡 Recommendations:  
  - Focus marketing campaigns on **low-revenue countries** to boost sales.  
  - Introduce **dynamic pricing strategies** in regions with high sales but low profitability.  
  - Strengthen **online sales channels** to capture emerging digital market opportunities.
 
  ---

  ## 📎 Dashboard  

